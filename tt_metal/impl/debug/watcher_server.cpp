...
std::string WatcherServer::Impl::log_file_name() {
    return tt::tt_metal::MetalContext::instance().rtoptions().get_output_dir() + LOG_FILE_PATH + LOG_FILE_NAME;
}
...
void WatcherServer::Impl::read_kernel_ids_from_file() {
    std::filesystem::path output_dir(tt::tt_metal::MetalContext::instance().rtoptions().get_output_dir() + LOG_FILE_PATH);
...
void WatcherServer::Impl::create_log_file() {
    ...
    std::filesystem::path output_dir(rtoptions.get_output_dir() + LOG_FILE_PATH);
